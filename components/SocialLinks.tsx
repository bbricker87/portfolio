...
const SocialLinks = ({ socials }: { socials: any }) => {
  const svgClass =
    'transition ease-in-out duration-500 w-6 h-6 sm:w-8 sm:h-8 fill-copper-red scale-100 hover:fill-cool-grey hover:scale-125'
  return (
    <ul className="flex justify-center align-middle my-4 sm:my-8">
      {socials?.length &&
        socials.map((social: any) => (
          <li className="mx-6" key={social.type}>
            <Link
